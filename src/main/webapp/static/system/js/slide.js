		document.querySelector('#table-demo-baseCode').GM({
		    ...
		    ajaxData: '/system/slide/datagrid',	//前台发送ajax的请求
		    ...
		    columnData: [
		    	{
		            ...
		            template: function(cell, row, index, key){
	                	return "<img src = '"+cell+"' width = '50px' alt = '无图片'/>";
	                }
		        },{
		            key: 'createDate',
		            text: '创建时间',
		...
		$(function(){
			...
			$("#table-demo-baseCode").on("click","a[data-id]",function(){
				...
				$("#delModelButton").on("click",function(){
					//get请求
					$.get("/system/slide/del",{"id":id},function(data){
    					if(data.success){
	    					$("#delModel").modal("hide");
	    					//刷新表格
			...
			$("#table-demo-baseCode").on("click","a[data-row]",function(){
				...
				//手动设置启用状态：clearForm()会将 启用状态 也清空
			...
			$("#saveButton").on("click",function(){
				//将表单以ajax异步请求的方式提交到后台，请求为：/system/slide/save
				$("#saveForm").ajaxSubmit({//ajax提交请求
	    			success:function(result){
	    				if(result.success){
	    					//关闭模态框
	    					$("#saveModel").modal("hide");
	    					//alert(result.msg);//测试能否获取返回参数
	    					//刷新表格
	    					GridManager.refreshGrid('demo-baseCode');
	    				}else{
